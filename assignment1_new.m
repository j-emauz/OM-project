...
tGA1= date2mjd2000(GA_window_1);
tGA2= date2mjd2000(GA_window_2);

tspan_dept= linspace(t11, t12, 100); %departure window
tspan_arrt = linspace(t21, t22, 100); %arrival window
% was 500
tspan_GA=linspace(tGA1, tGA2, 100); % fly-by window
...
for i=1:length(tspan_dept)
    for j=1:length(tspan_GA)       
        [dv_1(i,j),V_SC_Saturn_1(i,j),r_Saturn,t1(i,j),ToF1] = dv_arc1(tspan_dept(i), tspan_GA(j), p1, p2, mu_S);        
        ...
        for k=1:length(tspan_arrt) 
            ...
            dv_powered(i,j,k)= V_SC_Saturn_2 - V_SC_Saturn_1;
            % dv_2 manoeuver at asteroid plus 
            dv_tot(i,j,k)=dv_2(i,j,k)+dv_1(i,j)+dv_powered(i,j,k);
